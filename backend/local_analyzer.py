...
def analyze_contract_local(contract_text: str, language: str = "English") -> dict:
    ...
    for idx, block in enumerate(blocks):
        ...
        if re.search(r'indemnify', block_lower) or re.search(r'hold harmless', block_lower):
            found_clauses.append({
                ...
                "content": block,
                "risk_level": "Medium",
                "explanation": "Broad indemnification clauses can expose the MSME to uncapped liability, including third-party claims over which the vendor has no control.",
        ...
        if re.search(r'terminate.*convenience', block_lower) or re.search(r'without cause', block_lower):
            found_clauses.append({
                ...
                "content": block,
                "risk_level": "High",
                "explanation": "Allowing the corporation to terminate for convenience without cause leaves the MSME with unrecoverable sunk costs and inventory.",
        ...
        if re.search(r'entire agreement', block_lower) or re.search(r'supersedes all prior', block_lower):
            found_clauses.append({
                ...
                "content": block,
                "risk_level": "Medium",
                "explanation": "This clause legally wipes out any verbal assurances or side-emails the larger company used to convince the MSME to sign.",
        ...
        if re.search(r'as[- ]is', block_lower) and re.search(r'disclaim.*warranty', block_lower):
            found_clauses.append({
                ...
                "content": block,
                "risk_level": "Medium",
                "explanation": "Broad 'as-is' disclaimers shift all defect risk onto the party with less leverage.",
        ...
        if re.search(r'non-compete', block_lower) or (re.search(r'not.*manufacture', block_lower) and re.search(r'similar', block_lower)) or re.search(r'exclusive supplier', block_lower):
            found_clauses.append({
                ...
                "content": block,
                "risk_level": "High",
                "explanation": "Broad non-compete or exclusivity clauses restrict an MSME from serving other clients, often violating Section 27 of the Indian Contract Act.",
        ...
        if re.search(r'reserves the right.*to amend', block_lower) or re.search(r'unilaterally modify', block_lower):
            found_clauses.append({
                ...
                "content": block,
                "risk_level": "High",
                "explanation": "Allows the larger corporation to change the rules of the game (pricing, timelines) without the MSME's mutual written consent.",
    ...
    return {
        "summary": f"Offline Heuristic Analysis complete. The local AI dynamically scanned the uploaded document using heuristic rules and found {len(found_clauses)} predatory clauses matching known corporate exploitation patterns.",
        "risk_score": score,
